		...
		<div class="box">
			...
			<div class="box-body">	
				<?php 
					if (isset($_POST['save'])) {
                        session_start();
                        if ($_SESSION['login_admin']['role'] !== 'ADMIN') {
                            echo "<div class='alert alert-danger alert-dismissable' id='divAlert'>
                                    <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
                                    Anda tidak memiliki hak untuk menambah barang.
                                </div>";
                        } else {
                            // Save with status = 0 (pending approval)
                            ...
                            $hargab = $_POST['hargab'];
                            $stok = $_POST['stok'];
                            $stmt = $koneksi->prepare("INSERT INTO barang_pembelian(kd_pembelian, nama_barang_beli, satuan, harga_beli, item, status) VALUES(?, ?, ?, ?, ?, 0)");
                            $stmt->bind_param("sssdis", $kdbarang, $nama, $satuan, $hargab, $stok);
                            if ($stmt->execute()) {
                                echo "<div class='alert alert-info alert-dismissable' id='divAlert'>
                                        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
                                        Data tersimpan dan menunggu persetujuan manager.
                                    </div>";
                            } else {
                                echo "<div class='alert alert-danger alert-dismissable' id='divAlert'>
                                        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
