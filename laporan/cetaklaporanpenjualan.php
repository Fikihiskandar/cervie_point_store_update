...
$pemilik = $per['pemilik'];
$kota = $per['kota'];
$judul_H = "LAPORAN PENJUALAN BARANG";
$tgl = date('d-m-Y');
...
if (count($dat) === 0) {
	echo "<p><b>Tidak ada data penjualan untuk tanggal yang dipilih.</b></p>";
} else {
	foreach ($dat as $index => $data) {
		$no = $index + 1;
		$harga = (float) $data['harga_jual'];
		...
		echo "<tr>
			...
			<td align='center'>" . date_format(date_create($data['tgl_penjualan']), 'd-m-Y') . "</td>
